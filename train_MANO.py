...
for i in range(3):
    ...
    training_dataset = FerihandDatasetTrain(image_dir='./training/rgb', annotations_file='training_mano.json')
    training_data_loader = DataLoader(training_dataset, batch_size=64, shuffle=True)

    validation_dataset = FerihandDatasetValidation(image_dir='./evaluation/rgb', annotations_file='evaluation_mano.json')
    ...
    if i > 1:
        ...
        additional_layer = nn.Sequential(
            nn.Linear(model.fc.out_features, 512),  # First dense layer
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, 61)  # Second dense layer, outputting 61 features
        )
